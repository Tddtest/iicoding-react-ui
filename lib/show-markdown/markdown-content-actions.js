...
var getDirectory = (article) => {
  ...
  if (children) {
    ...
    const backAllSection = [...allSection];
    allSection = [];
    backAllSection.forEach((section) => {
      const isSideways = section.length > 1 && section.every((s) => s.type === section[0].type);
      if (isSideways) {
        allSection.push(...section.map((s) => [s]));
      } else {
